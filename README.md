...
- **AI 服务**: [DeepSeek API](https://deepseek.com/)
- **数据服务**: [RESTful API](https://restfulapi.net/) (标准接口规范)
...
### v1.3.0 - 商业级演示体验与合规性升级

- **全链路业务流程优化**：

  - **智能申报系统**：优化了 `Declaration Service`，实现从“填写表单”到“AI 核验”的全流程交互。用户提交申报后，系统会自动进行“上传云端”与“AI 识别”处理，并反馈核验结果（包含预估碳汇量）。
  - **数字化交易闭环**：完善了 `Transaction Service`，实现“购买 - 支付 - 存证”的完整闭环。支持本地数据持久化，确保用户操作记录的连续性与稳定性。
  - **沉浸式交互**：引入 `Loader2` 智能加载动画与 `Sonner` 实时状态提示，提升用户操作的流畅度与反馈感知。

- **合规性与文案优化**：

  - **证书编号生成**：实现了 `CN-2025-XXXX` 格式的电子证书编号生成逻辑，增强了平台的官方感与正规感。

- **商业计划书同步**：
  - 更新了《商业计划书》中的技术架构描述，使其与当前 **微服务架构** 的实现方案完全对齐，确保路演时“所见即所得”。

### v1.2.1 - 代码规范与体验修复

- **工程化重构**：

  - **图片资源集中管理**：创建 `src/constants/images.ts`，将全站分散的图片链接（包括 Picsum 随机图、纹理背景等）统一封装为常量，便于后续维护与替换。
  - **真实卫星图源**：将监测大屏 (`LiveMonitoring`) 的卫星底图替换为 Unsplash 高清真实遥感影像，提升视觉真实感。

- **Bug 修复**：
  - **导航栏抖动修复**：统一了 Navbar 在滚动前后的垂直内边距 (`py-3`)，彻底解决了滚动或切换页面时导航栏内容垂直跳动的问题。
  - **类型定义完善**：修复了交易平台 (`Platform.tsx`) 中项目详情弹窗的 TypeScript 类型缺失问题，确保构建流程无警告通过。
